...
// ─── 4. SAVE AS TEXT (.txt) ───
export const handleSaveAsText = (tiles: Tile[]) => {
  if (tiles.length === 0) {
    alert("Nothing to export!");
    return;
  }

  // 1. Calcolo dei confini della matrice attuale
  const minCol = Math.min(...tiles.map((t) => t.col));
  const maxCol = Math.max(...tiles.map((t) => t.col));
  const minRow = Math.min(...tiles.map((t) => t.row));
  const maxRow = Math.max(...tiles.map((t) => t.row));

  // Calcoliamo il numero effettivo di colonne utilizzate
  const columnsCount = maxCol - minCol + 1;

  // 2. Costruzione del contenuto testuale
  // Aggiungiamo l'intestazione richiesta come prima riga
  let textContent = `SET ${columnsCount} COLUMNS IN THE DROPDOWN\n\n`;

  for (let r = minRow; r <= maxRow; r++) {
    for (let c = minCol; c <= maxCol; c++) {
      const tile = tiles.find((t) => t.row === r && t.col === c);
      // Se esiste il tile mettiamo il carattere, altrimenti spazio
      textContent += tile ? tile.char : " ";
    }
    // A capo alla fine della riga
    textContent += "\n";
  }

  // 3. Richiesta nome file (Default: "22_columns")
  const defaultName = `${columnsCount}_columns`;
  let fileName = window.prompt("Enter a name for your text file:", defaultName);
